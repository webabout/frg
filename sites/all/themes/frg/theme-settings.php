...
include_once(drupal_get_path('theme', 'frg') .'/inc/template.theme-settings.inc');
...
*   A form array.
*/
function frg_settings($saved_settings, $subtheme_defaults = array()) {
  ...
  $defaults = frg_theme_get_default_settings('frg');
  ...
  if ($settings['layout_enable_settings'] == 'on') {
    $image_path = drupal_get_path('theme', 'frg') .'/css/core-images';
  ...
  if (module_exists('views')) {
    $form['themedev']['dev']['classses']['views_classes'] = array(
      ...
      '#description' => t('NOTE: If you are using custom Views templates you must use the template overrides that come with frg to preserve this functality.'),
      '#collapsible' => TRUE,
      '#collapsed' => TRUE,
    );
